...
3. Move the card to reflect reality. Locate the board the way `next` §1 does.
   ...
   - Label fallback: create `status:in-review` first if missing (`gh label create status:in-review --force` — see `next` §1), then swap `status:in-progress` for it (complete) or leave as `status:in-progress` (partial/blocked), and add a comment explaining why if blocked.
