...
3. **Candidates:** Todo/Ready items (cross-checks `gh issue list` if the eventually-consistent `gh project item-list` returns empty), or `status:todo` issues, or open issues in roadmap order. Sorted by priority label, else roadmap order, else oldest first. A repo's `CLAUDE.md`/`AGENTS.md` rules override all of this.
4. **Present:** top 1–3 with title, number, size (`size:*` label or "unestimated"), suggested model (fast for well-specified, strong for ambiguous/`needs-design`). "go" picks the top one.
