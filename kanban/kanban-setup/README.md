...
2. **Board (idempotent):** looks for `<repo> Board` like `next` §1; creates it with `gh project create` if missing; confirms the default Status field (Todo / In Progress / Done) via `gh project field-list`.
...
5. **Preview, then create:** shows every issue (title, mode, target column) and waits. Then `current` → create + Todo; `retroactive` → create, close, Done.
